...
// This is an AI-modified version of powerpass.ts which demonstrates AES encryption and decryption
// Locker object in the web edition of PowerPass
...
export interface Securable {
    ...
    decrypt: (source: string, key: string) => Promise<void>;
    newSecret: () => Secret;
}
...
export class PowerPassLocker implements Locker, Securable {
    ...
    async encrypt(key: string): Promise<string> {
        ...
        // Derive a 256-bit key from the provided string (SHA-256)
        const enc = new TextEncoder();
        const keyMaterial = enc.encode(key);
        ...
        // Return a JSON string containing iv and ciphertext (base64 encoded)
        const payload = {
            v: 1,
    ...
    async decrypt(source: string, key: string): Promise<void> {
        ...
        // Try to parse as our encrypted payload first
        try {
            ...
            if (parsed && parsed.iv && parsed.data) {
                ...
                const plainBuf = await crypto.subtle.decrypt({ name: 'AES-GCM', iv: new Uint8Array(ivBuf) }, cryptoKey, dataBuf);
                const plaintext = dec.decode(plainBuf);
                this.secrets = JSON.parse(plaintext);
                return;
            }
        } catch (e) {
            // fall through to try legacy/plain JSON parsing
        }
